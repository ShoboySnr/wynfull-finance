...
(function () {
    ...
    function collectAnswers() {
        ...
        const checkedSituations = Array.from(document.querySelectorAll('input[name="financial-situation"]:checked')).map(i => i.value);
        const situationOther = document.getElementById('financial-situation-other')?.value || null;

        const primaryGoal = document.querySelector('input[name="primary-goal"]:checked')?.value || '';
        const primaryGoalOther = document.getElementById('primary-goal-other')?.value || null;

        const confidence = document.querySelector('input[name="confidence-level"]:checked')?.value || '';
        const debtFeeling = document.querySelector('input[name="debt-feeling"]:checked')?.value || '';
        const savingsAmount = document.querySelector('input[name="savings-amount"]:checked')?.value || '';
        const investingStatus = document.querySelector('input[name="investing-status"]:checked')?.value || '';
        const investingExperience = document.querySelector('input[name="investing-experience"]:checked')?.value || '';

        return {
            financial_situation: checkedSituations,
            financial_situation_other: situationOther,
            primary_goal: primaryGoal,
            primary_goal_other: primaryGoalOther,
            confidence_level: confidence,
            debt_feeling: debtFeeling,
            savings_amount: savingsAmount,
            investing_status: investingStatus,
    ...
    function isLastStep() { return currentStep === totalSteps; }
    ...
    nextBtn?.addEventListener('click', async function () {
        ...
        if (!isLastStep()) {
            currentStep++;
            // showStep(currentStep)...
            if (currentStep === totalSteps) nextBtn.textContent = 'Finish';
        ...
        try {
            ...
            const res = await fetch(window.WYNFULL.onboardingCompleteRoute, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': window.WYNFULL.csrfToken,
            ...
            if (!res.ok) {
                ...
                throw new Error(data.message || 'Unable to save onboarding');
            }
